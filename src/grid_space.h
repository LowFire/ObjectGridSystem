...
class GridSpace : public RefCounted {
	...
	private:
	int _id_counter = 0;
	HashMap<int, GridObject*> _grid_objects;
	...
	private:
	void _on_object_dimensions_changed(GridObject *p_obj, Vector2i p_old_dimensions, Vector2i p_new_dimensions);
	void _on_object_position_changed(GridObject *p_obj, Vector2i p_old_position, Vector2i p_new_position);
	...
	GridObject* get_object_by_id(int id) const {
		if (!_grid_objects.has(id)) return nullptr;
		return _grid_objects[id];
	}
	bool has_object(const GridObject* p_obj) {
		for(KeyValue<int, GridObject*> &E : _grid_objects) {
	...
	void remove_object(GridObject* p_obj);
